...
const signup = async (req, res) => {
  ...
  // Vérification des entrées faites par l'admin

  if (alias === null || email === null || password === null) {
    return res.status(400).json("Missing inputs ! Please retry.");
  }

  try {
    const query = "SELECT alias, email, password FROM admin WHERE email = ?";
    const isAdmin = await Query.findByValue(query, email);
...
const signin = async (req, res) => {
  try {
    ...
    const query1 =
      "SELECT id, alias, email, password FROM admin WHERE email = ?";
